## (!) is for Platform Types
If you are interoperating with Java code from Kotlin, then there may be old Java code that does not have any annotations
about the return type (whether it's `@Nullable` or `@NotNull`). So Kotlin use the `!` to denote it as a **Platform Type**.

This means that the developer must make the decision of how to handle the nullability of that returned variable.

It's appropriate to do something like this:
...
department may in turn have another employee as a department head. To obtain the name of Bob's department head (if there
is one), you write the following:
...
throws an exception if the value is `null`. You can write `b!!`, and this will return a non-null value of `b` (for example, a
`String` in our example) or throw an NPE if `b` is `null`:
